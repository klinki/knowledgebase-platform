...
| \*\*1.4\*\* | \*\*\[Optional] Shadow API\*\* | Implement a background worker to intercept `/\_\_graphql/` requests. Match the JSON response objects to the current Tweet ID for richer metadata collection. |
...
| \*\*3.1\*\* | \*\*Database Schema\*\* | Design tables: `raw\_captures` (source data) and `processed\_insights` (refined data). |
...
\* \*\*Accuracy:\*\* AI-generated tags match user intent  of the time.

\* \*\*Reliability:\*\* Extension successfully renders the button on  of loaded tweets.
